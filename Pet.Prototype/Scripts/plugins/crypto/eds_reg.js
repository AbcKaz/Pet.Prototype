...
function fillPersonData(data) {
    ...
    if (document.getElementById('signFIO2')) {
        document.getElementById('signFIO2').innerHTML = fullName;
        document.getElementById('signFIO2').value = fullName;
    }
}
...
function checkCompany(bin) {    

    $.post(rootUrl + "Account/CheckCompany", { bin: bin }, function (data) {
        console.log("check bin=", data);
        if (data.IsExist == true) {

            $('#signBIN').val(data.BIN);
            $('.juridical-address').val(data.JuridicalAddress);
            $('.fact-address').val(data.FactAddress);
...
function checkUser(iin) {

    $.post(rootUrl + "Account/CheckUser", { iin: iin }, function (data) {
        console.log("check bin=", data);
        if (data.IsExist == true) {
            $('#UserIsExist').val("1");
            toastr["error"]("Вы уже зарегистрирован");
        } else {           
            checkCompany($('#signBIN').val());
        }
    });
}
